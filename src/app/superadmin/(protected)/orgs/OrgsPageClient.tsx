...
// Status indicator: a single coloured dot (green = live, red = offline).
// Replaced the old LIVE / OFFLINE chip — quieter, reads at a glance.
  ...
  return (
    <span
      ...
      style={{
        ...
        boxShadow: live ? '0 0 0 3px color-mix(in srgb, var(--sa-success) 25%, transparent)' : 'none',
      }}
    />
  )
}
...
function OrgRow({ org, empCount, leadCount, isFirst }: RowProps) {
  ...
  // Quota state — shown as a small chip when configured
  ...
  return (
    <div
      ...
      style={{
        ...
        // Single subtle left bar for at-risk orgs — no row tinting
        boxShadow: overLimit
      ...
      <Link href={`/superadmin/orgs/${org.id}`} className="flex items-center gap-3.5 flex-1 min-w-0">
        {org.logo_url ? (
          <img
            ...
            style={{ background: '#fff', border: '1.5px solid var(--sa-border)' }}
          />
        ) : (
          <div
    ...
    <div className="relative min-h-screen p-4 md:p-10" style={{ color: 'var(--sa-text)' }}>
      <div className="relative max-w-4xl mx-auto" style={{ zIndex: 10 }}>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-end sm:justify-between mb-8 md:mb-10 gap-4">
          <div>
            <p className="text-[10px] font-bold tracking-[0.3em] uppercase mb-2" style={{ color: 'var(--sa-text-muted)' }}>
              Workspace management
            </p>
            <h1 className="text-3xl sm:text-4xl font-black tracking-tight" style={{ color: 'var(--sa-text)' }}>
              Organisations
            </h1>
          </div>
          <Link
            href="/superadmin/orgs/new"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all self-end sm:self-auto"
            style={{
        ...
        {/* Stat tiles — uniform white surface + black border + black offset.
            One palette throughout, value colour comes from text-text not accent. */}
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            { label: 'Total orgs',      value: regularOrgs.length },
            ...
            >
              <p className="text-2xl font-black tabular-nums" style={{ color: 'var(--sa-text)' }}>{s.value}</p>
              <p className="text-[11px] mt-0.5 font-bold" style={{ color: 'var(--sa-text-secondary)' }}>{s.label}</p>
            </div>
          ))}
        </div>

        {/* Sandbox group — single amber accent on the strip, not on every row */}
        ...
        {regularOrgs.length === 0 ? (
          <div
            ...
            style={{
              ...
              border: '2px dashed var(--sa-shadow-color)',
            }}
          >
